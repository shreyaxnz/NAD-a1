...
#include <unistd.h>
	...
	public:
		...
		{
			if (empty()) //If it is empty, put it back in the line
			{
				push_back(p);
			}
			else
			{
				if (!sequence_more_recent(p.sequence, front().sequence, max_sequence)) //If it not recent than front-packet
				{
					push_front(p); //then put in front
				}
				...
				{
					for (PacketQueue::iterator itor = begin(); itor != end(); itor++) //checks where to put 
					{
						assert(itor->sequence != p.sequence); //checks packets doesnt have duplicate sequence numbers
		...
		void verify_sorted(unsigned int max_sequence)
		{
			...
			for (PacketQueue::iterator itor = begin(); itor != end(); itor++) //checks through packet queue
			{
				assert(itor->sequence <= max_sequence); //checks the sequence number is within range
				if (prev != end())
				{    //it should be in increasing order only
	...
	public:
		...
		//This function is called when we send packet with one arguement
		void PacketSent(int size)
		{
			...
			//Checking is the packet is not already sent or pending
			...
			data.time = 0.0f;
			data.size = size;
			//Push the data of the packet
			sentQueue.push_back(data);
			pendingAckQueue.push_back(data);
			sent_packets++; //Increment it 
			local_sequence++;
			if (local_sequence > max_sequence) //update the sequence number, if it is more than max sequence number
				local_sequence = 0; //it resets to 0
		}

		//This function is called when we receive packet
		...
		//This function generates acknowledgement for received packets
		unsigned int GenerateAckBits()
		{
			return generate_ack_bits(GetRemoteSequence(), receivedQueue, max_sequence);
		}
		//This function processes the acknowledgement
		...
		//This function updates reliability
		...
		//This function validates that queues are sorted according to it's numbe
		void Validate()
		{
